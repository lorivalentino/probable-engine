...
def move(source_x, source_y, destination_x, destination_y):
    if source_x < 0 or source_x > 15:
        raise Exception("Invalid source x-coordinates")
    if source_y < 0 or source_y > 15:
...
while not is_finished:
    try:
        ...
        # print "Input source x-coordinate:"
        # sourcexinput = raw_input()
        # print "Input source y-coordinate:"
        # sourceyinput = raw_input()
        # print "Input destination x-coordinate:"
        # destxinput = raw_input()
        # print "Input destination y-coordinate:"
        # destyinput = raw_input()
        player_input = raw_input()

        if player_input.split()[0] == "save":
            save_state(player_input.split()[1])
            continue
        if player_input.split()[0] == "load":
            load_state(player_input.split()[1])
            draw()
            continue
        source_x, source_y, dest_x, dest_y = player_input.split()
        move(int(source_x), int(source_y), int(dest_x), int(dest_y))
        ...
        if count_coins() > 0:
            ...
            print "Player", ActivePlayer+1, "has won!"
            is_finished = True
